"""Extract Whisper encoder hidden-state embeddings per phoneme token.

Reads tokens.csv + features_acoustic.csv (for wav_path), runs Whisper forward
...
# --------------------------------------------------------------------------
# Constants (Whisper architecture, not tunable)
# --------------------------------------------------------------------------
SAMPLE_RATE = 16000        # Whisper expects 16 kHz mono
FRAME_RATE_HZ = 50.0       # encoder output rate after conv stride
...
# --------------------------------------------------------------------------
# Whisper forward + layer extraction
# --------------------------------------------------------------------------
def encode_wav(
    audio: np.ndarray,
    ...
    # Index 0 = post-conv embedding, 1..N = transformer layer outputs.
...
def main():
    ...
    nw = cfg["extract_neural_whisper"]

    tokens_path = Path(nw["input_tokens"])
    feats_path = Path(nw["input_features_acoustic"])
    model_name = nw["model_name"]
    layers = list(nw["layers"])
    device = resolve_device(nw["device"])
    dtype_str = nw["embedding_dtype"]
    out_dir = Path(nw["output_dir"])
    out_prefix = nw["output_prefix"]
    out_paths = {L: out_dir / f"{out_prefix}_L{L:02d}.npz" for L in layers}
    debug = nw.get("debug", {"enable": False})
